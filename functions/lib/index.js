...
// Define Secrets
const secretsConfig = (0, params_1.defineSecret)("FUNCTIONS_CONFIG_EXPORT");
...
exports.createStripeProduct = functions.firestore
    ...
    if (collectionName !== 'trainingCourses' && collectionName !== 'supervisionGroups')
        return;
...
exports.stripeWebhook = functions.https.onRequest(async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        const hasSpecialistAccess = ((_a = session.metadata) === null || _a === void 0 ? void 0 : _a.hasSpecialistAccess) === 'true';
        try {
            if (itemType === 'trainingCourse' && courseId && userId) {
                ...
                await db.runTransaction(async (t) => {
                    ...
                    const currentParticipants = ((_a = courseDoc.data()) === null || _a === void 0 ? void 0 : _a.participantIds) || [];
                ...
                await db.runTransaction(async (t) => {
                    ...
                    const currentMembers = ((_a = groupDoc.data()) === null || _a === void 0 ? void 0 : _a.memberIds) || [];
...
exports.sendMailOnCreate = functions
    ...
    if (!data || !data.to || !data.subject || !data.html) {
        console.error("Invalid mail document.");
        return;
    }
    try {
        const { email: senderEmail } = getSmtpConfig();
...
const createConfirmationEmail = (userName, itemName, type, detailsHtml) => ({
...
exports.onTrainingJoin = functions
    .runWith({ secrets: [secretsConfig] })
    ...
    if (!before || !after)
        return;
    if ((after.participantIds || []).length <= (before.participantIds || []).length)
        return;
    const newUserId = after.participantIds.find((id) => !before.participantIds.includes(id));
    if (!newUserId)
        return;
    const user = (await db.collection("users").doc(newUserId).get()).data();
    if (!(user === null || user === void 0 ? void 0 : user.email))
        return;
    const formatDate = (ts) => ts ? new Date(ts.seconds * 1000).toLocaleDateString("en-GB", { weekday: 'short', day: 'numeric', month: 'short', year: 'numeric' }) : "";
    const details = `<h3>Course Details</h3><p><strong>Trainer:</strong> ${after.trainerName}</p><p><strong>Venue:</strong> ${after.venueName}</p><p><strong>Schedule:</strong><br/>Core: ${(((_a = after.dates) === null || _a === void 0 ? void 0 : _a.core) || []).map(formatDate).join("<br/>")}<br/>Specialist: ${(((_b = after.dates) === null || _b === void 0 ? void 0 : _b.specialist) || []).map(formatDate).join("<br/>")}<br/>Supervision: ${(((_c = after.dates) === null || _c === void 0 ? void 0 : _c.supervision) || []).map(formatDate).join("<br/>")}</p>`;
    const email = createConfirmationEmail(user.displayName, after.name, 'Training', details);
    await db.collection("mail").add(Object.assign({ to: user.email }, email));
});
exports.onSupervisionJoin = functions
    .runWith({ secrets: [secretsConfig] })
    ...
    if (!before || !after)
        return;
    if ((after.memberIds || []).length <= (before.memberIds || []).length)
        return;
    const newUserId = after.memberIds.find((id) => !before.memberIds.includes(id));
    if (!newUserId)
        return;
    const user = (await db.collection("users").doc(newUserId).get()).data();
    if (!(user === null || user === void 0 ? void 0 : user.email))
...
exports.onUserStatusUpdate = functions
    ...
    // ELSA certification approved
        ...
        try {
            ...
            console.error("Line manager notification failed", e);
        }
    }
...
exports.onCourseCompletion = functions.firestore
    ...
    if (requiredSessions === 0)
        return; // Not a valid course to check
    ...
    const potentiallyCompletedUsers = new Set();
    ...
    for (const userId of potentiallyCompletedUsers) {
        ...
        if (attendedCount >= requiredSessions) {
            ...
            if (userSnap.exists && ((_a = userSnap.data()) === null || _a === void 0 ? void 0 : _a.role) === 'Trainee') {
...
exports.createTrainingChat = functions.firestore
    .document("trainingCourses/{courseId}")
    .onCreate(async (snap, context) => {
...
exports.createSupervisionChat = functions.firestore
...
exports.syncTrainingChatMembers = functions.firestore
    .document("trainingCourses/{courseId}")
    .onUpdate(async (change, context) => {
    var _a;
    const after = change.after.data();
    ...
    if (JSON.stringify(after.participantIds) === JSON.stringify(before.participantIds))
        return;
    const chatId = context.params.courseId;
    const chatRef = db.collection("chats").doc(chatId);
    const chatDoc = await chatRef.get();
    if (!chatDoc.exists)
        return; // Should exist, but fail safe
    const currentAdmins = ((_a = chatDoc.data()) === null || _a === void 0 ? void 0 : _a.adminIds) || [];
...
exports.syncSupervisionChatMembers = functions.firestore
    .document("supervisionGroups/{groupId}")
    .onUpdate(async (change, context) => {
    var _a;
    const after = change.after.data();
    ...
    if (JSON.stringify(after.memberIds) === JSON.stringify(before.memberIds))
        return;

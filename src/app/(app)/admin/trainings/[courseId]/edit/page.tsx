  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      ...
      if (docSnap.exists()) {
        ...
        const coreDates = data.dates?.core?.map((d: Timestamp) => d.toDate()) || [];
        const specialistDates = data.dates?.specialist?.map((d: Timestamp) => d.toDate()) || [];
        const supervisionDates = data.dates?.supervision?.map((d: Timestamp) => d.toDate()) || [];
        
        const isDatesTbc = data.datesTbc ?? (coreDates.length === 0 && specialistDates.length === 0 && supervisionDates.length === 0);
        ...
        form.reset({
          ...
          specialistDates,
          supervisionDates,
          specialistPrice: data.specialistPrice || 0,
          datesTbc: isDatesTbc,
        });
      } else {
  ...
  return (
    <RoleGate allowedRoles={['Admin', 'Trainer']}>
      <div className="container max-w-2xl py-10">
        <Card>
          <CardHeader>
            <CardTitle>Edit Training Course</CardTitle>
          </CardHeader>
          <CardContent>
            <Form {...form}>
